...
describe("isWeekend", () => {
	test("Saturday and Sunday are the weekend", () => {
		...
		expect(isWeekend(ms(2024, 3, 30))).toBe(true);
		expect(isWeekend(ms(2024, 3, 31))).toBe(true);
	});

	test("Monday through Friday are not", () => {
		// April 1 2024 (Mon) through April 5 (Fri).
		for (let day = 1; day <= 5; day++) {
			expect(isWeekend(ms(2024, 4, day))).toBe(false);
		}
	});
});
...
describe("addBusinessDays skips weekends", () => {
	test("Friday plus one lands on Monday", () => {
		...
		expect(addBusinessDays(ms(2024, 3, 29), 1, undefined, NO_CAP)).toBe(ms(2024, 4, 1));
	});

	test("a full working week lands a calendar week on", () => {
		expect(addBusinessDays(ms(2024, 3, 29), 5, undefined, NO_CAP)).toBe(ms(2024, 4, 5));
	});

	test("zero is the identity, even starting on a weekend", () => {
		expect(addBusinessDays(ms(2024, 3, 29), 0, undefined, NO_CAP)).toBe(ms(2024, 3, 29));
		expect(addBusinessDays(ms(2024, 3, 30), 0, undefined, NO_CAP)).toBe(ms(2024, 3, 30));
	});

	test("negative retreats over the weekend the same way", () => {
		// Monday April 1 minus one working day is the Friday before.
		expect(addBusinessDays(ms(2024, 4, 1), -1, undefined, NO_CAP)).toBe(ms(2024, 3, 29));
	});

	test("a fractional count is truncated to whole working days", () => {
		expect(addBusinessDays(ms(2024, 3, 29), 1.9, undefined, NO_CAP)).toBe(ms(2024, 4, 1));
	});
});
...
describe("addBusinessDays skips holidays too", () => {
	test("Christmas and Boxing Day are stepped over", () => {
		...
		expect(addBusinessDays(ms(2024, 12, 24), 1, isHoliday, NO_CAP)).toBe(ms(2024, 12, 27));
	});

	test("the same span with no calendar stops at the 25th", () => {
		expect(addBusinessDays(ms(2024, 12, 24), 1, undefined, NO_CAP)).toBe(ms(2024, 12, 25));
	});
	...
	test("a holiday on the landing day pushes one more forward", () => {
		...
		expect(addBusinessDays(ms(2024, 12, 20), 1, isHoliday, NO_CAP)).toBe(ms(2024, 12, 24));
	});
});
...
describe("addBusinessDays respects the step cap", () => {
	test("a calendar that marks every day a holiday returns null rather than looping", () => {
		const everyDay = () => true;
		expect(addBusinessDays(ms(2024, 1, 1), 1, everyDay, 500)).toBeNull();
	});
	...
	test("an offset needing more calendar days than the cap allows returns null", () => {
		...
		// never reach it.
		expect(addBusinessDays(ms(2024, 3, 29), 1, undefined, 0)).toBeNull();
	});

	test("an in-cap offset still returns the date", () => {
		expect(addBusinessDays(ms(2024, 3, 29), 1, undefined, 3)).toBe(ms(2024, 4, 1));
	});
});
...
describe("countBusinessDaysBetween is inclusive and order-independent", () => {
	test("every working day in January 2024, both ends counted", () => {
		...
		expect(countBusinessDaysBetween(ms(2024, 1, 1), ms(2024, 1, 31), undefined, NO_CAP)).toBe(23);
	});

	test("the two dates may be written in either order", () => {
		expect(countBusinessDaysBetween(ms(2024, 1, 31), ms(2024, 1, 1), undefined, NO_CAP)).toBe(23);
	});

	test("a single weekday is one, a single weekend day is zero", () => {
		expect(countBusinessDaysBetween(ms(2024, 4, 1), ms(2024, 4, 1), undefined, NO_CAP)).toBe(1);
		expect(countBusinessDaysBetween(ms(2024, 3, 30), ms(2024, 3, 30), undefined, NO_CAP)).toBe(0);
	});

	test("a single working week is five", () => {
		// Monday April 1 through Sunday April 7 2024.
		expect(countBusinessDaysBetween(ms(2024, 4, 1), ms(2024, 4, 7), undefined, NO_CAP)).toBe(5);
	});
	...
	test("holidays inside the window are not counted", () => {
		...
		const isHoliday = holidaysFrom([2024, 12, 25], [2024, 12, 26]);
		expect(countBusinessDaysBetween(ms(2024, 12, 23), ms(2024, 12, 27), isHoliday, NO_CAP)).toBe(3);
		expect(countBusinessDaysBetween(ms(2024, 12, 23), ms(2024, 12, 27), undefined, NO_CAP)).toBe(5);
	});
	...
	test("the time of day on either endpoint does not tip the count", () => {
		...
		expect(countBusinessDaysBetween(lateInDay, ms(2024, 1, 31), undefined, NO_CAP)).toBe(23);
	});

	test("a span wider than the cap returns null", () => {
		expect(countBusinessDaysBetween(ms(2024, 1, 1), ms(2024, 1, 31), undefined, 5)).toBeNull();
	});
});
