...
/** A document that sets one of everything, so a highlight has state to disturb. */
const DOCUMENT = [
	"total += 5",
	":x = 3",
	"y = 3",
...
/**
 * Every shape of line that changes something when it runs, and the ones the
 * audit for #559 found beyond the issue's own `total += 5`.
 */
const STATE_CHANGING_LINES: [string, string][] = [
...
/** The internals a highlight used to write, read without widening their visibility. */
...
function describeValue(value: Value | null | undefined): string {
	return value ? `${value.type}:${formatValue(value)}` : "none";
}

/** Everything a line can change when it runs, as plain comparable data. */
function stateOf(engine: ExpressionEngine, doc?: DocumentModel) {
	...
	const documentResults: string[] = [];
	if (doc) {
		for (let n = 1; n <= doc.lineCount; n++) documentResults.push(describeValue(doc.getLineAt(n)?.result));
	}
	return {
		variables: vm.getVariableEntries().map(([name, value]) => `${name} = ${describeValue(value)}`).sort(),
		functions: vm.getUserFunctionDefs().map(def => `${def.name}(${def.params.join(", ")})`).sort(),
...
function batch() {
	...
	engine.parseDocument(DOCUMENT.join("\n"));
	return { engine, doc: undefined, dispose: () => engine.clear() };
}

/** A live editor: a document model, an incremental evaluator, several settled passes. */
function editor(lines: string[] = DOCUMENT) {
	const engine = createEngine() as unknown as ExpressionEngine;
...
describe("what a live editor shows after highlighting", () => {
	/** The answers a reader sees, formatted. */
	const answers = (doc: DocumentModel) =>
