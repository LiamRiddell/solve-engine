...
import { TagBreakdownParselet } from "./parselets/TagBreakdownParselet";
import {
  tagAggregateNormalizerRule,
  tagBreakdownNormalizerRule,
  tagStripNormalizerRule,
} from "./normalizer/TagAggregateNormalizerRule";
import {
  tagSumHandler,
  tagAverageHandler,
  tagCountHandler,
 ...
 * `total above` and the table-column aggregates cannot reach. `total by tag`
 * breaks the whole note down by every tag at once, each with its share.
 *
 ...
 * fusions (`total of`, `count of`, `average of`) and the raw `sum of`, and the
 * breakdown needs all of `total by tag`, so bare `total`/`sum`/`count`/`average`
 * and `tag` never become keywords and `:total = 5` is untouched.
 */
export const TAGS_PACKAGE: IEnginePackage = {
  name: "solve-tags",
  normalizerRules: [tagAggregateNormalizerRule(), tagBreakdownNormalizerRule(), tagStripNormalizerRule()],
  prefixParselets: {
    TAG_SUM: new TagAggregateParselet("sum"),
    TAG_AVERAGE: new TagAggregateParselet("average"),
    TAG_COUNT: new TagAggregateParselet("count"),
    TAG_BREAKDOWN: new TagBreakdownParselet(),
  },
  pluginFunctions: {
    sum: tagSumHandler,
    average: tagAverageHandler,
    count: tagCountHandler,
    tagBreakdown: tagBreakdownHandler,
  },
};
