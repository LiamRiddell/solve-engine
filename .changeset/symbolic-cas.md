...
`expand`, `factor`, `solve`, `der`/`derivative`, `integral`, `taylor` and `jacobian` are new, along with a bare equation form: write `x^2-4 = 0` on one line and ask for `x =>` on a later one. Factoring and solving work over the rationals, so `x^2-2` and `x^2+1` come back unchanged rather than being approximated, and an irrational root is given as a square root rather than a decimal.
