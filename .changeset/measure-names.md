...
A new suite names every measure in both unit tables through its representative unit, checks that the table of representatives covers every measure so a new one cannot slip past, and sweeps sums, products and conversions of each against a length and a mass, asserting that no refusal contains a camelCase measure key. `npm run verify:ci` passes: TESTS tests across SUITES suites, with the bundled-consumer contract.
