...
A plain number is given a unit the same way a quantity is converted, with `in`:
`5 in km` is five kilometres. The word after `in` has to be a unit, a currency or
the name of a converter, so a number is never labelled with a word that is none
of those. A converter's name reads as the converter, as it does after `as`, so
`2024 in roman` is the Roman numeral.

```solve-doc
5 in km // 5.00 km
5 in EUR // €5.00
5 in widgets // ERROR: "widgets" is not a unit.
