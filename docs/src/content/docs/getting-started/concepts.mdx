...
`createEngine()` registers forty-four packages; three more (stocks, crypto and
knowledge) are opt-in because they need you to supply how to fetch the data. A bare
`new ExpressionEngine()` registers none, so you pass exactly the packages you want.
