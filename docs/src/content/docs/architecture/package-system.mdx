...
const engine = new ExpressionEngine("en", false, undefined, undefined, packages);
```
...
It saves no bundle size at all. Registration happens at runtime, while what ends
up in your bundle is decided by imports, and those are static. The symbolic
package's grammar is about eight kilobytes of the engine's brotli-compressed
total, and that code ships whether you register it or not, because the virtual
machine's builtin table imports the algebra functions directly. Filtering the
array changes which words parse, not which code loads.
