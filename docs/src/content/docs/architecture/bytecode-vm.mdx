...
numeric and string constants. A constant-pool operand is a single byte, which
caps each pool at 256 entries, checked at compile time.
...
are configurable and both produce a named error, `INSTRUCTION_LIMIT_EXCEEDED` and
`STACK_LIMIT_EXCEEDED`. This matters because the input
is untrusted and arrives one keystroke at a time.

<Explainer id="dispatch" client:load />
...
<Explainer id="stackValues" client:load />
