...
| --- | --- |
| `import { ExpressionEngine }`, the core with no package registered | {kb(sizes.importOneGzip)} |
| the whole root entry, close to what `createEngine()` pulls in | {kb(sizes.importEverythingGzip)} |
