...
fixed cost of standing an engine up is spread across more lines. It eases back
on the two largest documents, and there the warm re-parse is no faster than the
cold one. That is the compile cache running out of room, not the processor: it
holds 2,000 compiled lines by default (see [Caching, in layers](#caching-in-layers)
below), each of the two largest benchmark documents holds more distinct lines
than that, and a document that does not fit compiles most of its lines again on
every pass. A host whose documents run that long raises
`config.performance.defaultCacheSize` to hold them.

The number to take from the table is its size: an engine meant to run on every
keystroke keeps pace with a document far larger than anyone types into one, and
re-parsing an unchanged document that fits in the cache is quicker still, since
its bytecode is already compiled. The sections below are why.
...
(see [Incremental re-evaluation](#incremental-re-evaluation) below): there an
...
- `parseDocument` runs every line on each call, and skips only the compile
  stages for text it has seen before.
- `evaluateDocument` is a one-off pass: it builds a fresh evaluator for the
  call, runs every line, and discards the evaluator.
- A long-lived `ThreeTierEvaluator`, told about each edit and given the range
  of lines on screen (the viewport), runs the edited line, the lines above the
  viewport that transitively depend on it through the dependency graph, and the
  visible lines. An unchanged line above the viewport is not run at all, and
  the lines below it wait until they are scrolled into view.

Measured over a 1,000-line note in which every line calls a counting function,
after editing only its last line:
...
The caches keep a re-parse cheaper than a first parse. They do not move the
...
`eventTargetTransport` takes anything with the `EventTargetLike` shape: a DOM
...
of time, so scrolling does not pay for it. That compilation moves to a worker
when there is one.
...
differently. So the engine asks. Register a factory once and the compile path uses it.
...
Load-time performance is parse time, and parse time is bytes: before the first
...
Two measured figures bound what an import costs. Both come from bundling the
engine for real on every commit with size-limit, which bundles with rolldown and
compresses the result with brotli, as a web server would send it, and both are
read here from the same data the home page quotes rather than typed in:

| Import | Minified and compressed |
| --- | --- |
| `import { ExpressionEngine }`, the core with no package registered | {kb(sizes.importOneBrotli)} |
| the whole root entry, close to what `createEngine()` pulls in | {kb(sizes.importEverythingBrotli)} |
...
two, and where it lands depends on the bundler. Under rolldown it leaves out a
real share of the built-ins it never imports. Under esbuild it saves almost
nothing today: the built-in packages share chunks of the published build, and
esbuild keeps those chunks whole, so an arithmetic-only engine bundles to
nearly the size of `createEngine()`.

The boundary: until the build is split so that each package stands on its own,
a slim engine is not a size saving under esbuild, and a host bundling with it
gains little by leaving packages out. The split, and a table generated from it
for each bundler, are open work. `createEngine` stays the plain choice when
bundle size is not the concern. Each [syntax page](/syntax/cheatsheet/) names
the package its feature needs, so a slim build registers exactly what it uses.
See [choosing packages](/getting-started/installation/).
