...
and selected by name from the main side. A synchronous or pending result crosses
in the DTO; streaming the async resolver's later live-data events across the
boundary is not part of this first slice.
